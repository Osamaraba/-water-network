...
def _geo_to_records(gdf):
    """تحويل GeoDataFrame إلى قائمة قواميس مع geometry بصيغة WKT."""
    if gdf is None or gdf.empty:
        return None
    d = gdf.copy()
    if "geometry" in d.columns:
        d["_wkt"] = d["geometry"].apply(lambda g: g.wkt if g is not None else None)
    ...
    if kml_file and st.session_state.customers_gdf is None:
        ...
        with st.spinner("جاري قراءة KML..."):
            try:
                ...
                with tf.NamedTemporaryFile(suffix=".kml", delete=False) as tmp:
